...
for d = 1:12    % Derivaciones
    ...
    try
        ...
        limit.N2 = round(max(tags.qrsoff - tags.qrs)) + 300; % QT fisiológico(400)
        if isnan(limit.N1) && ~isnan(limit.N2)
            limit.N1 = 200; % PQ fisiológico(200) + 50ms
        elseif isnan(limit.N2) && ~isnan(limit.N1)
            limit.N2 = 300; % QT fisiológico(400) + 50ms
        elseif isnan(limit.N2) && isnan(limit.N1)
            limit.N1 = 250; % PQ fisiológico(200) + 50ms
            limit.N2 = 300; % QT fisiológico(400) + 50ms
        end
